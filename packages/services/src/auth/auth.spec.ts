...
import { local } from '@template-web/toolkit/dom/local';
import type { Cookies } from '@template-web/toolkit/dom/cookies';

import Auth, { AuthMethods } from './auth';

jest.mock('@template-web/toolkit/dom/local', () => ({
    local: {
        remove: jest.fn(),
    },
}));

jest.mock('@template-web/toolkit/dom/cookies', () => {
    return {
